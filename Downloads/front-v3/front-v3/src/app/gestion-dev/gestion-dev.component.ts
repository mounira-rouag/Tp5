...
export class GestionDevComponent implements OnInit {
  ...
  duplicateRow(originalDev: Dev): void {


    const duplicatedDev: Dev = { ...originalDev };

    console.log("before duplication", this.alldev);
    //this.alldev.push(duplicatedDev);
    console.log("after duplication", this.alldev);
    //this.alldev = [...this.alldev];

    this.devService.duplicateDev(originalDev).subscribe(
      duplicatedDev => {
        this.alldev.push(duplicatedDev);
        // Handle the duplicatedDev response as needed
        console.log('Duplicated Dev:', duplicatedDev);
      },
      error => {
        // Handle error if the duplication process fails
