...
const httpOptions = {
  ...
  export class DevService {
      ...
      return forkJoin(devStreams).pipe(
        map(devArrays => {
          ...
          const filteredDevs = distinctDevs.filter(dev =>
            (!marqueId || dev.vehicules.find((vehicule :any) => vehicule.grpMarque == marqueId)) &&
            (!idmaj || dev.maj.idMaj == idmaj) &&
            (!siteId || dev.user?.site == siteId) &&
            (!codeVeh || dev.vehicules.find((vehicle:any) => vehicle.codeVeh == codeVeh))
          );
    ...
    registerDev(DevRequest: any): Observable<any> {
      return this.http.post<any>(`${environment.apiUrl}/dev/add`, DevRequest);
    } 
    ...
    DuplicateDev( updatedDev: Dev,versionCode:any,siteCode:any, rcCode:any,checkMenu:any,options:any,textBug:any,IsAdapConserved:any,textcomment:any): Observable<Dev> {
      return this.http.post<Dev>(`${environment.apiUrl}/dev/duplicate/${versionCode}/${siteCode}/${rcCode}/${checkMenu}/${options}/${textBug}/${IsAdapConserved}/${textcomment}`, updatedDev); 
    }
