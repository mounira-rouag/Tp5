...
export class GestionDevComponent implements OnInit {
 ...
 model!:Vehicule;
  ...
  duplicate(data:any){
    const url = "duplicate-dev/"+data.id
  ...
  exportPersonalisedToExcel() {
    ...
    const exportData = this.alldev.map(dev => ({
      ...
      bug:dev.bug ? dev.bug:'',
       ...
       Vehicule:this.model.nomVeh ||'not found ',
       commentaire: dev.devComment ? dev.devComment:'',
    }));
   ...
   GetVehiculeById(){
    this.vehiculeService.getVehichulesById(this.selectemodel).subscribe(res=>{
      this.model=res;
      
      console.log("model",this.model.nomVeh);
    })
   }
  exportStanderToExcel() {
    // Prepare export data
    const exportData = this.alldev.map(dev => ({
      Id: dev.id,
      DevName: dev.devname,
  ...
  getMarqueById(){
    this.marqueService.getMarqueById(this.selectedMarque).subscribe(res=>{
       this.marque=res;
       console.log(this.marque.nomMarque);
