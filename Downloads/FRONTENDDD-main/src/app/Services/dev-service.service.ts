    ...
    getAllDev(): Observable<any> {
      ...
      console.log (devStreams);
        ...
        return forkJoin(devStreams).pipe(
          map(devArrays => {
            const uniquesdevs=new Set<Dev>();
            console.log ("this is dev array ",devArrays)
            /** */
            const filteredDevs = devArrays.flat().filter(dev =>
              ...
              (!codeVeh ||
                 dev.vehicules?.find(vehicle => vehicle.codeVeh === codeVeh)) 
              )
            );  
            console.log("those filtered devs f",filteredDevs)
            return filteredDevs;
          }),
          catchError((error) => {
